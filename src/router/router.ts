...
export class Router {
  ...
  private async loadTemplate(templatePath: string): Promise<void> {
    try {
      ...
      // Check for either theme toggle button and initialize if not already done
      const homeThemeToggle = document.getElementById('theme-toggle');
      const navThemeToggle = document.getElementById('nav-theme-toggle');
      const themeToggle = homeThemeToggle || navThemeToggle;

      if (themeToggle && !themeToggle.hasAttribute('data-initialized')) {
        const { themeConfiguration } = await import(
          '../functionality/toggle-theme'
        );
        themeConfiguration();
