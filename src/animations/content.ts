...
import {
  getAllElementsByDataAnimation,
  getElementByDataAnimation,
} from './data-animation';
...
export const whyWeExistAnimation = () => {
  ...
  const paragraphs = getAllElementsByDataAnimation(
    LOCATION,
    'why-we-exist-paragraph'
  );

  if (!box || !title || !paragraphs) {
    return;
  }
  ...
  growTimeline.to(box, { autoAlpha: 1, duration: 1.0, ease: 'power2.out' });
