...
export const copyPhone = (document: Document) => {
    ...
    const copyButton = document.querySelector<HTMLButtonElement>(copyButtonSelector);
...
export const notifyAnimation = (document: Document, element: HTMLElement) => {
    ...
    const notifyContainerElement = document.querySelector<HTMLElement>(notifyContainerSelector);
    ...
    timeline
        ...
        .to(notifyElement, { y: "100%", duration: 0.35, ease: 'power2.in' }, holdInSeconds)
        .add(() => element.remove());
}
